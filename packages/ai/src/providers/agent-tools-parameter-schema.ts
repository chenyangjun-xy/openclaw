...
function resolveToolParameterSchemaCacheKey(
  ...
  const capGbnfMaxLength = options?.modelCompat?.capGbnfMaxLength === true;
...
function normalizeToolParameterSchemaUncached(
  ...
  // llama.cpp GBNF grammar enforces a ~2000 repetition ceiling. Providers
  // that use llama.cpp under the hood (ollama, llamacpp, etc.) auto-enable
  // the cap so schemas compile without error.
  const isLlamaCppProvider =
    normalizedProvider.includes("llama") ||
    normalizedProvider.includes("ollama") ||
    options?.modelCompat?.capGbnfMaxLength === true;
