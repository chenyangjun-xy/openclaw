...
describe("normalizeToolParameterSchema GBNF cap", () => {
  ...
  describe("auto-detect llama.cpp providers", () => {
    it("caps when provider is ollama (auto-detected)", () => {
      const schema = normalizeToolParameterSchema(
        { type: "object", properties: { s: { type: "string", maxLength: 65_536 } } },
        { modelProvider: "ollama" },
      );
      const fields = collectStringFields("root", schema as SchemaLike);
      expect(fieldMaxLength(fields, "root.s")).toBe(GBNF_SAFE_CEILING);
    });

    it("caps when provider name contains llama (auto-detected)", () => {
      const schema = normalizeToolParameterSchema(
        { type: "object", properties: { b: { type: "string", maxLength: 100_000 } } },
        { modelProvider: "llama-cpp" },
      );
      const fields = collectStringFields("root", schema as SchemaLike);
      expect(fieldMaxLength(fields, "root.b")).toBe(GBNF_SAFE_CEILING);
    });

    it("caps when ollama provider without explicit compat flag", () => {
      const schema = normalizeToolParameterSchema(
        { type: "object", properties: { x: { type: "string", maxLength: 80_000 } } },
        { modelProvider: "ollama", modelCompat: {} },
      );
      const fields = collectStringFields("root", schema as SchemaLike);
      expect(fieldMaxLength(fields, "root.x")).toBe(GBNF_SAFE_CEILING);
    });

    it("does NOT cap when provider is openai (not GBNF)", () => {
      const schema = normalizeToolParameterSchema(
        { type: "object", properties: { x: { type: "string", maxLength: 65_536 } } },
        { modelProvider: "openai" },
      );
      const fields = collectStringFields("root", schema as SchemaLike);
      expect(fieldMaxLength(fields, "root.x")).toBe(65_536);
    });
  });
